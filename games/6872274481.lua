vape:CreateNotification('please fuh me in the ass, mommy.', 30)
...
local antihitrange = nil
local antihitgroundtime = nil
local antihitairtime = nil
local antihitautoair = nil
local antihitsettings = {}

local upOffset = Vector3.new(0, 60, 0)
local downOffset = Vector3.new(0, -60, 0)
...
local function createClone()
    if entitylib.isAlive and entitylib.character.Humanoid.Health > 0 and (not oldroot or not oldroot.Parent) then
        ...
        entitylib.character.Humanoid.HipHeight = hip
    end
end

local function destroyClone()
    if oldroot and oldroot.Parent == gameCamera then
        ...
        for _, v in lplr.Character:GetDescendants() do
            if v:IsA("Weld") or v:IsA("Motor6D") then
                if v.Part0 == oldroot then v.Part0 = oldroot end
                if v.Part1 == oldroot then v.Part1 = oldroot end
            end
        end
    end
end
        ...
        if call then
            antihit:Clean(runService.PreSimulation:Connect(function()
                if entitylib.isAlive and tick() > FlyLandTick then
                    ...
                    if oldroot and oldroot.Parent then
                        oldroot.CFrame = CFrame.new(unpack(cf)) + Vector3.new(0, 6, 0)
                    end
                end
            end))
            ...
            repeat
                ...
                if entitylib.character.AirTime and plr and (tick() - entitylib.character.AirTime) < 2 or projectileHitting then
                    createClone()
                    tpbackup = not tpbackup
                    antihitting = not tpbackup
                    projectileHitting = false

                    if antihitting and oldroot then
                        oldroot.CFrame += upOffset
                        task.delay(1, function()
                            if antihitting and oldroot and oldroot.Parent then
                                oldroot.CFrame += downOffset
                            end
                        end)
                    end
                else
...
antihitsettings = antihit:CreateDropdown({
    Name = 'Target',
    List = {'Players', 'NPCs'},
    Default = {'Players'},
    Multi = true
