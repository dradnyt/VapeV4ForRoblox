...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/dradnyt/VapeV4ForRoblox/main/'..select(1, path:gsub('newvape/', '')), true)
		end)
		...
		if (not teleportedServers) and (not shared.VapeIndependent) then
			...
			local teleportScript = [[
				...
				if shared.VapeDeveloper then
					...
					loadstring(game:HttpGet('https://raw.githubusercontent.com/dradnyt/VapeV4ForRoblox/main/loader.lua', true), 'loader')()
				end
			]]
			if shared.VapeDeveloper then
